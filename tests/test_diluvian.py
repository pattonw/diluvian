...
from diluvian.util import binary_confusion_matrix, confusion_f_score, get_nonzero_aabb
...
def test_octree_bounds():
    ...
    ot[
        clip_bounds[0][0] : clip_bounds[1][0],
        clip_bounds[0][1] : clip_bounds[1][1],
        clip_bounds[0][2] : clip_bounds[1][2],
    ] = 6
    assert isinstance(
        ot.root_node, octrees.UniformNode
    ), "Constant assignment should make root uniform."

    ot[8, 5, 4] = 5
    expected_mat = np.array([[[6], [6]], [[6], [5]]], dtype=np.uint8)
    assert np.array_equal(
        ot[7:9, 4:6, 4], expected_mat
    ), "Assignment should break uniformity."

    expected_types = [
        [[octrees.BranchNode, None], [None, None]],
        ...
        for j, row in enumerate(col):
            for k, expected_type in enumerate(row):
                if expected_type is None:
                    assert (
                        ot.root_node.children[i][j][k] is None
                    ), "Clip bounds should make most nodes empty."
                else:
                    assert isinstance(
                        ot.root_node.children[i][j][k], expected_type
                    ), "Nodes are wrong type."

    np.testing.assert_almost_equal(
        ot.fullness(),
        2.0 / 3.0,
        err_msg="Octree fullness should be relative to clip bounds.",
    )

    ot[10, 5, 4] = 5  # Break the remaining top-level uniform branch node.
    np.testing.assert_almost_equal(
        ot.fullness(), 1.0, err_msg="Octree fullness should be relative to clip bounds."
    )

    np.testing.assert_array_equal(
        ot.get_leaf_bounds()[1],
        clip_bounds[1],
...
def test_octree_map_copy():
    ...
    for orig, copy in zip(ot.iter_leaves(), cot.iter_leaves()):
        np.testing.assert_almost_equal(
            copy.bounds[0],
            orig.bounds[0],
            err_msg="Copy leaves should have same bounds.",
        )
        np.testing.assert_almost_equal(
            copy.bounds[1],
            orig.bounds[1],
        ...
        np.testing.assert_almost_equal(
            copy.data, leaf_map(orig.data), err_msg="Copy leaves should be mapped."
        )
    expected_mat = np.array([[[9.], [-6.]], [[9.], [-5.]]], dtype=np.float32)
    assert np.array_equal(
        cot[7:9, 4:6, 4], expected_mat
    ), "Copy should have same uniformity."
...
def test_region_moves():
    ...
    for i, move in enumerate(
        ...
        coord = (
            ctr
            + (region.MOVE_DELTA * move)
            + np.array([2, 2, 2]) * (np.ones(3) - np.abs(move))
        )
...
def test_volume_transforms():
    ...
    np.testing.assert_array_equal(
        dpv.local_coord_to_world(np.array([2, 2, 2])), np.array([8, 8, 34])
    )
    np.testing.assert_array_equal(
        dpv.world_coord_to_local(np.array([8, 8, 34])), np.array([2, 2, 2])
    )

    svb = volumes.SubvolumeBounds(
        np.array((0, 0, 32), dtype=np.int64), np.array((4, 4, 33), dtype=np.int64)
    )
...
def test_volume_transforms_image_stacks():
    ...
    svb = volumes.SubvolumeBounds(
        np.array((2420, 0, 0), dtype=np.int64), np.array((2421, 4, 4), dtype=np.int64)
    )
    sv = v.get_subvolume(svb)
...
def test_volume_identity_downsample_returns_self():
    ...
    v = volumes.Volume(
        resolution, image_data=np.zeros((1, 1, 1)), label_data=np.zeros((1, 1, 1))
    )
