        ...
        if epoch == self.threshold_epoch:
            ...
            if current >= self.threshold_value:
                raise EarlyAbortException(
                    "Aborted after epoch {} because {} was {} >= {}".format(
                        self.threshold_epoch,
                        self.monitor,
                        current,
...
def augment_subvolume_generator(subvolume_generator):
    ...
    for v in CONFIG.training.augment_noise:
        gen = GaussianNoiseAugmentGenerator(
            gen, CONFIG.training.augment_use_both, v["axis"], v["mul"], v["add"]
        )
    for v in CONFIG.training.augment_artifacts:
        if "cache" not in v:
            v["cache"] = {}
        gen = MaskedArtifactAugmentGenerator(
            gen,
            CONFIG.training.augment_use_both,
            v["axis"],
            v["prob"],
            v["volume_file"],
            v["cache"],
    ...
    for v in CONFIG.training.augment_missing_data:
        gen = MissingDataAugmentGenerator(
            gen, CONFIG.training.augment_use_both, v["axis"], v["prob"]
        )
    for v in CONFIG.training.augment_contrast:
        gen = ContrastAugmentGenerator(
            gen,
            CONFIG.training.augment_use_both,
            v["axis"],
            v["prob"],
...
class MovingTrainingGenerator(six.Iterator):
    ...
    def __init__(
        ...
        subv_per_epoch=None,
        subv_metric_fn=None,
        subv_metric_threshold=False,
        subv_metric_args=None,
    ):
    ...
    def get_epoch_metric(self):
        assert (
            len(self.epoch_subv_metrics) == self.subv_per_epoch
        ), "Not all validation subvs completed: {}/{} (Finished moves: {}, ongoing: {})".format(
            len(self.epoch_subv_metrics),
            self.subv_per_epoch,
            self.epoch_move_counts,
            self.move_counts,
        )
    ...
    def __next__(self):
        ...
        if all(self.fake_mask):
            inputs = collections.OrderedDict(
                {
                    "image_input": np.repeat(
                        pad_dims(self.fake_block["image"]),
                        CONFIG.training.num_gpus,
                        axis=0,
                    ),
                    "mask_input": np.repeat(
                        pad_dims(self.fake_block["mask"]),
                        CONFIG.training.num_gpus,
                        axis=0,
                    ),
                }
            ...
            outputs = np.repeat(
                pad_dims(self.fake_block["target"]), CONFIG.training.num_gpus, axis=0
            )
        ...
        if (
            active_regions
            and self.kludge["outputs"] is not None
            and self.kludge["inputs"] is not None
        ):
            for n in active_regions:
                assert np.array_equal(
                    self.kludge["inputs"][n, :], self.batch_image_input[n, 0, 0, :, 0]
                )
                self.regions[n].add_mask(
                    self.kludge["outputs"][n, :, :, :, 0], self.region_pos[n]
                )
        ...
        inputs = collections.OrderedDict(
            {"image_input": self.batch_image_input, "mask_input": batch_mask_input}
        )
        inputs["kludge"] = self.kludge
        ...
        if self.f_a_bins is None:
            ...
            if self.f_a_init:
                ...
                sample_weights = np.reciprocal(
                    self.f_a_counts[f_a_inds], dtype=np.float64
                ) * float(self.f_as.size)
            return (inputs, [batch_mask_target], sample_weights)


DataGenerator = collections.namedtuple(
    "DataGenerator", ["data", "gens", "callbacks", "steps_per_epoch"]
)


def get_output_margin(model_config):
    return np.floor_divide(
        model_config.input_fov_shape - model_config.output_fov_shape, 2
    )
...
def build_validation_gen(validation_volumes):
    ...
    if len(validation_volumes) == 1:
        ...
        validation_volumes = {
            "dupe {}".format(n): single_vol for n in range(CONFIG.training.num_workers)
        }

    validation_gens = [
        preprocess_subvolume_generator(
            v.subvolume_generator(
                shape=CONFIG.model.validation_subv_shape, label_margin=output_margin
            )
        )
    ...
    validation_kludges = [
        {"inputs": None, "outputs": None} for _ in range(CONFIG.training.num_workers)
    ]
    validation_data = [
        MovingTrainingGenerator(
        ...
        for i, (gen, kludge) in enumerate(
            zip(validation_worker_gens, validation_kludges)
        )
    ]
    ...
    VALIDATION_STEPS = np.ceil(
        CONFIG.training.validation_size / CONFIG.training.batch_size
    )
    # Number of all-move sequences must be a multiple of number of worker gens.
    VALIDATION_STEPS = np.ceil(VALIDATION_STEPS / len(validation_worker_gens)) * len(
        validation_worker_gens
    )
    VALIDATION_STEPS = VALIDATION_STEPS * CONFIG.model.validation_subv_moves + len(
        validation_worker_gens
    )
...
def build_training_gen(training_volumes):
    ...
    training_gens = [
        augment_subvolume_generator(
            preprocess_subvolume_generator(
                v.subvolume_generator(
                    shape=CONFIG.model.training_subv_shape, label_margin=output_margin
                )
            )
        )
    ...
    TRAINING_STEPS_PER_EPOCH = (
        CONFIG.training.training_size // CONFIG.training.batch_size
    )
...
def train_network(
    ...
    logging.info(
        "Using {} volumes for training, {} for validation.".format(
    ...
    if (
        CONFIG.training.early_abort_epoch is not None
        and CONFIG.training.early_abort_loss is not None
    ):
        callbacks.append(
            EarlyAbort(
                threshold_epoch=CONFIG.training.early_abort_epoch,
    ...
    if viewer:
        ...
        for inputs, targets in viz_ex:
            ...
            output_offset = np.array(inputs["image_input"].shape[1:4]) - np.array(
                targets[0].shape[1:4]
            )
            output_offset = np.flipud(output_offset // 2)
            viewer.add(inputs["image_input"][0, :, :, :, 0], name="Image")
            viewer.add(
                inputs["mask_input"][0, :, :, :, 0],
                name="Mask Input",
