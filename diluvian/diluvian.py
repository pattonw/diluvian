...
from .util import get_color_shader, Roundrobin, WrappedViewer
from .volumes import HDF5Volume, partition_volumes, SubvolumeBounds
...
def generate_subvolume_bounds(filename, volumes, num_bounds, sparse=False, moves=None):
    if "{volume}" not in filename:
        raise ValueError(
            'CSV filename must contain "{volume}" for volume name replacement.'
        )
...
def fill_volume_with_model(
    ...
    def worker(
        worker_id, set_devices, model_file, image, seeds, results, lock, revoked
    ):
        lock.acquire()
        ...
        while True:
            ...
            region = Region(
                image, seed_vox=seed, sparse_mask=True, block_padding="reflect"
            )
    ...
    if filter_seeds_by_mask and volume.mask_data is not None:
        seeds = [
            s for s in seeds if volume.mask_data[tuple(volume.world_coord_to_local(s))]
        ]

    pbar = tqdm(desc="Seed queue", total=len(seeds), miniters=1, smoothing=0.0)
    label_pbar = tqdm(
        desc="Labeled vox", total=prediction.size, miniters=1, smoothing=0.0, position=1
    )
    num_seeds = len(seeds)
    if shuffle_seeds:
    ...
    while dispatched_seeds:
        ...
        for seed in dispatched_seeds:
            if (
                np.all(bounds[0] <= seed)
                and np.all(bounds[1] > seed)
                and mask[tuple(seed - bounds[0])]
            ):
                loading_lock.acquire()
...
def fill_volumes_with_model(
    ...
    **kwargs
    ...
    if "{volume}" not in filename:
        raise ValueError(
            'HDF5 filename must contain "{volume}" for volume name replacement.'
        )
    if resume_filename is not None and "{volume}" not in resume_filename:
        raise ValueError(
            'TOML resume filename must contain "{volume}" for volume name replacement.'
        )
    ...
    for volume_name, volume in six.iteritems(volumes):
        ...
        if resume_filename is not None:
            ...
            resume_volume = six.next(
                six.itervalues(HDF5Volume.from_toml(resume_volume_filename))
            )
        ...
        config = HDF5Volume.write_file(
            volume_filename + ".hdf5", CONFIG.volume.resolution, label_data=prediction
        )
...
def fill_region_with_model(
    ...
    if bounds_input_file is not None:
        gen_kwargs = {
            k: {
                "bounds_generator": iter(
                    SubvolumeBounds.iterable_from_csv(
                        bounds_input_file.format(volume=k)
                    )
                )
            }
...
def evaluate_volume(
    ...
    def labels_to_cremi(v):
        ...
        if hasattr(v, "bounds"):
            label_data = label_data[
                list(map(slice, list(v.bounds[0]), list(v.bounds[1])))
            ]
    ...
    if np.any(pred_upsample > 0):
        ...
        logging.warn(
            "Segmentation is different resolution than groundtruth. Upsampling by %s.",
            scale,
        )
        ...
        if hasattr(pred_vol, "bounds"):
            pred_data = pred_data[
                list(map(slice, list(pred_vol.bounds[0]), list(pred_vol.bounds[1])))
            ]
        orig_shape = pred_data.shape
        pred_data = np.lib.stride_tricks.as_strided(
            pred_data,
        ...
        if np.any(padding > 0):
            logging.warn(
                "Padding segmentation (%s) to be groundtruth size (%s)",
                new_shape,
                gt_vol.shape,
            )
    ...
    if use_gt_mask and gt_vol.mask_data is not None:
        logging.warn(
            "Groundtruth has a mask channel that will be applied to segmentation."
        )
        mask_data = gt_vol.mask_data
        if hasattr(gt_vol, "bounds"):
            mask_data = mask_data[
                list(map(slice, list(gt_vol.bounds[0]), list(gt_vol.bounds[1])))
            ]
        ...
        if relabel:
            ...
            new_pred = morphology.label(
                pred_copy, background=background_label_id, connectivity=2
            )
