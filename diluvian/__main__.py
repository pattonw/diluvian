...
def _make_main_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Train or run flood-filling networks on EM data."
    )

    commandparsers = parser.add_subparsers(help="Commands", dest="command")

    train_parser = commandparsers.add_parser(
        "train", parents=[common_parser], help="Train a network from labeled volumes."
    )
    train_parser.add_argument(
        "-mo",
        "--model-output-filebase",
        dest="model_output_filebase",
        default=None,
    ...
    fill_parser = commandparsers.add_parser(
        "fill",
        parents=[common_parser, fill_common_parser],
    ...
    fill_parser.add_argument(
        ...
        default=None,
        type=int,
        help="Cancel filling after this many bodies (only useful for " "diagnostics).",
    )
    fill_parser.add_argument(
        "--reject-early-termination",
        action="store_true",
        dest="reject_early_termination",
        default=False,
        help="Reject seeds that terminate early, e.g., due to maximum " "move limits.",
    )
    fill_parser.add_argument(
        "--resume-file",
        dest="resume_filename",
        default=None,
    ...
    validate_parser = commandparsers.add_parser(  # noqa
        "validate", parents=[common_parser], help="Run a model on validation data."
    )
    ...
    evaluate_parser.add_argument(
        "ground_truth_name", default=None, help="Name of the ground truth volume."
    )
    evaluate_parser.add_argument(
        "prediction_name", default=None, help="Name of the prediction volume."
    )
    ...
    view_parser.add_argument(
        ...
        dest="partition_volumes",
        default=False,
        help="Partition volumes and view centered at the validation " "partitions.",
    )
    view_parser.add_argument(
        "volume_name_regex",
        default=".",
    ...
    check_config_parser = commandparsers.add_parser(
        "check-config", parents=[common_parser], help="Check a configuration value."
    )
    ...
    gen_subv_bounds_parser = commandparsers.add_parser(
        "gen-subv-bounds",
        parents=[common_parser, bounds_common_parser],
    ...
    gen_subv_bounds_parser.add_argument(
        "num_bounds", default=None, type=int, help="Number of bounds to generate."
    )
...
def main():
    ...
    if args.command == "train":
        ...
        while True:
            try:
                ...
                if args.early_restart:
                    ...
                    logging.warning(
                        "Training aborted, restarting with random seed %s", new_seed
                    )
                    init_seeds()
                    continue
                else:
        ...
        volumes = load_volumes(
            args.volume_files, args.in_memory, name_regex=args.volume_name_regex
        )
